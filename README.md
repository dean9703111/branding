...
```bash
npm run build:branding            # 重新產生 branding/index.html
...
├── assets/         ← 圖片資產（形象照、書封、截圖、活動照、og.png）
...
| 區塊 | 內容 |
|---|---|
| `# 網站設定` | 分頁標題、SEO 描述、`url`（canonical／OG 網址）、`og_image`（社群預覽圖路徑）、`favicon`（圖示網址） |
...
- 部署上線：把 `index.html` 與 `assets/` 一起上傳即可，路徑都是相對的
